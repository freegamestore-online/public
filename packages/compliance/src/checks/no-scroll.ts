 ...
 * detect "is this a game project" by looking for the @freegamestore/games
...
export async function checkNoScroll(source: FileSource): Promise<CheckResult> {
  ...
  if (issues.length > 0) {
    return {
      ...
      suggestions: [
        ...
        'Use <GameShell> from @freegamestore/games — it locks layout to 100svh.',
  ...
  if (!sawViewportLock) {
    return {
      ...
      suggestions: [
        'Wrap your game in <GameShell> from @freegamestore/games, or',
...
const VIEWPORT_LOCK = /(?:height|max-height)\s*:\s*100(?:s?vh)|GameShell|@freegamestore\/games/i;
