...
const GAME_DEPS = JSON.stringify({
  name: 'my-game',
  dependencies: { '@freegamestore/games': '^0.1.0' },
});
...
describe('checkNoScroll', () => {
  ...
  it('detects @freegamestore/games dep as the game signal', async () => {
    const dir = await fixture({
      'package.json': GAME_DEPS,
      'web/src/index.css': 'body { height: 100svh; }',
      'web/src/App.tsx': 'import { GameShell } from "@freegamestore/games";',
    });
    const r = await checkNoScroll(fsFileSource(dir));
