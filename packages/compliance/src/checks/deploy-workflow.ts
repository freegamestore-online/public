 ...
 * by `fgs create` and rarely needs customisation.
 *
...
export async function checkDeployWorkflow(source: FileSource): Promise<CheckResult> {
  ...
  return {
    ...
    suggestions: [
      ...
      'Run `fgs create` to scaffold a game with the standard deploy workflow, or copy one from an existing game repo.',
    ],
  };
}
